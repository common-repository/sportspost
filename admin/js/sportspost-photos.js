	...
	*/
		...
		render: function () {
			...
			$( img ).load( function() {
				...
				$( this ).fadeIn( function() { 
					if ( that.model.get( 'connected' ) == true ) {
						$( '#sportspost-insert-button' ).removeAttr( 'disabled' ); 
	...
	wp.media.controller.SportsPostMedia = wp.media.controller.State.extend({
		...
		importAction: function() {
			...
			$( '#sportspost-insert-button' ).attr( 'disabled', 'disabled' );
		...
		insertAction: function(){
			if ( $( '#sportspost-insert-button' ).is( '[disabled=disabled]' ) ) return;
			$( '#sportspost-insert-button' ).attr( 'disabled', 'disabled' );
			$( '#sportspost-import-button' ).attr( 'disabled', 'disabled' );
			$( '#sportspost-insert-button' ).text( 'Inserting...' );
			...
			$.when.apply( this, jqHXRs ).done( function() {
				$( '#sportspost-insert-button' ).text( wp.media.view.l10n.sportspostInsertButton );
				$( '#sportspost-insert-button' ).removeAttr( 'disabled' );
				$( '#sportspost-import-button' ).removeAttr( 'disabled' );
	...
	wp.media.view.Toolbar.SportsPostMedia = wp.media.view.Toolbar.extend({
				...
				items: {
					...
					sportspost_event_disconnect: {
						...
						click: function( e ) {
							...
							if ( r == true ) {
								$( '#sportspost-spin-status' ).css( 'visibility', 'visible' ).show();
								$.post(
									ajaxurl,
									{ 
		...
		refresh: function() {
			...
			var show = false;
			if ( selection.length > 0 && connected == true ) show = true;
...
/*			if ( this.model.get( 'initialized' ) ) {
		...
		createToolbar: function() {
			...
			$( '#sportspostspin' ).css( 'visibility', 'visible' ).show();
			var filter = this.$el.find( '.sportspost' ),
				sportspostmsg = $( '<div/>', {
				id: 'sportspost-msg'
			});
		...
		setButtonStates: function( selection ) {
			var show = false;
			if ( selection != '' && selection.length > 0 && this.model.get( 'connected' ) == true ) show = true;
			if ( ! show ) {
				$( '#sportspost-insert-button' ).attr( 'disabled', 'disabled' );
				$( '#sportspost-import-button' ).attr( 'disabled', 'disabled' );
				$( '#sportspost-import-button' ).text( wp.media.view.l10n.sportspostImportButton );
		...
		getImages: function( collection, source, method, param, page, altpage, paramlabel, photoset ) {
			this.$el.find( '#sportspostspin' ).css( 'visibility', 'visible' ).show();
